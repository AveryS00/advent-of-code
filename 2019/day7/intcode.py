class IntcodeProcessor:
    ...
    def __init__(self, intcode, inputArr=None):
    ...
    def runOpcode(self):
        while self.index < len(self.intcode):
            ...
            op = self.readCode()
            if op[0] == 1:
                self.opAdd(op[1], op[2])
            elif op[0] == 2:
                self.opMultiply(op[1], op[2])
            elif op[0] == 3:
                self.intcode[self.intcode[self.index+1]] = self.inputArr[self.inputIdx]
                self.inputIdx += 1
                self.index += 2
            elif op[0] == 4:
                ...
                self.opLessThan(op[1], op[2])
            elif op[0] == 8:
                self.opEquals(op[1], op[2])
            elif op[0] == 99:
                return 0
            else:
                print('Invalid input')
    ...
    def opAdd(self, param1, param2):
        self.intcode[self.intcode[self.index+3]] = self.getVal(self.index+1, param1) + self.getVal(self.index+2, param2)
        self.index += 4
        
    def opMultiply(self, param1, param2):
        self.intcode[self.intcode[self.index+3]] = self.getVal(self.index+1, param1) * self.getVal(self.index+2, param2)
        self.index += 4
    ...
    def opLessThan(self, param1, param2):
        if self.getVal(self.index+1, param1) < self.getVal(self.index+2, param2):
            self.intcode[self.intcode[self.index+3]] = 1
        else:
            self.intcode[self.intcode[self.index+3]] = 0
        self.index += 4
        
    def opEquals(self, param1, param2):
        if self.getVal(self.index+1, param1) == self.getVal(self.index+2, param2):
            self.intcode[self.intcode[self.index+3]] = 1
        else:
            self.intcode[self.intcode[self.index+3]] = 0
